...
| Passing and blocking evidence packets | [`reviewer-evidence-packet.md`](reviewer-evidence-packet.md) | Recording the reproducible `Security Reviewed` or `Published only` evidence before writing the short decision note. |
| Review decision note | [`review-decision-note.md`](review-decision-note.md) | Turning the evidence packet into a concise PR, issue, or review-record outcome. |
| Source-backed verification | [`source-backed-verification.md`](source-backed-verification.md) | Checking API, package, platform, permission, or security-control claims and unsupported-claim actions against reviewable sources. |
